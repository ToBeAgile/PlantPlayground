...
sensor_read_frequency = 120.0 #Hz
network_write_frequency = 20 #Hz    How many data points will be graphed each second
...
def read_sensor():
    ...
    while True:
        ...
        a_value = (adc.read_adc_difference(0, gain=a_gain, data_rate=128)) * mv_per_division
        a_time = datetime.datetime.now().strftime("%H:%M:%S:%f")
        #read_event.wait(0.01) #this is probably what's causing the problems
        b_value = (adc.read_adc_difference(3, gain=b_gain, data_rate=128)) * b_mv_per_division
